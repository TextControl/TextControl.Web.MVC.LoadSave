...
}var h=f?i+window.location.host+"/"+TXSettings.ApiRoute+"TXPrint":i+b.getHTTPHandlerRootDir()+"TXPrintHandler.ashx";
...
break;
case d.ArrowNE:if((b.BrowserDetect.browser==="Explorer")||(b.BrowserDetect.browser==="Other")||(b.BrowserDetect.browser==="Edge")){r="url("+(b.isSSLConnection?"https://":"http://")+window.location.host+"/TXTextControl.Web/cursors/markline.cur), default";
...
break;
case d.DownArrow:if((b.BrowserDetect.browser==="Explorer")||(b.BrowserDetect.browser==="Other")||(b.BrowserDetect.browser==="Edge")){r="url("+(b.isSSLConnection?"https://":"http://")+window.location.host+"/TXTextControl.Web/cursors/tablecol.cur), default";
...
e.load("/TXTextControl.Web/Dialogs/"+this.DialogInfo.FileName,this.loadedBound);
};
...
g.load("/TXTextControl.Web/Dialogs/InputBoxDialog.html",this.loadedBound);
};
return d;
})();
return c;
...
h.appendChild(i);
var g=a("#"+this.dialogContainerID);
g.load("/TXTextControl.Web/Dialogs/"+this.DialogInfo.FileName,this.loadedBound);
this.callback=f;
};
...
this.dialogContainer.load("/TXTextControl.Web/Dialogs/TxDialog.html",this.loadedBound);
};
return d;
})();
return c;
...
break;
case"mnuItemShowTextFrameMarkerLines":console.log(R.item.context.id+" clicked.");
var P=a("#tglBtnShowTextFrameMarkerLines");
...
var TXTextControl=(function(aF,a){var b=aF._private=aF._private||{};
aF.version="23.0";
function ar(){delete aF._private;
...
var aE={VariousMessages:0,RibbonStrings:1};
var aH={Unknown:0,ServiceNotInstalled:1,ServiceNotRunning:2,CouldNotConnectToService:3};
var r={Left:false,Middle:false,Right:false};
...
function q(){return("ontouchend" in document);
}b.isTouchDevice=q();
b.isSocketConnected=function(){return b.socket&&(b.socket.readyState==WebSocket.OPEN);
};
b.sendCommand=function(aM,aO,aP,aQ,aN){if(!b.isSocketConnected()){return;
}var aL={CommandID:parseInt(aM,10),Param1:aO||0,Param2:aP||0,Param3:aQ||0,Data:aN||"",};
var aR=JSON.stringify(aL);
b.socket.send(aR);
};
b.getHTTPHandlerRootDir=function(){var aO="";
var aN=window.location.pathname.split("/");
aN.pop();
var aL=aN.join("/")+"/";
if((typeof TXWebResources!=="undefined")&&TXWebResources.hasOwnProperty("webSocketHandlerPath")){if(TXWebResources.webSocketHandlerPath){var aM=TXWebResources.webSocketHandlerPath;
if(aM.slice(-1)!=="/"){aM+="/";
}if(aM[0]==="/"){aO=window.location.host+aM;
}else{aO=prefix+window.location.host+aL+aM;
}}}else{aO=window.location.host+aL;
}return aO;
};
function p(){var aL=(typeof TXSettings!=="undefined")&&TXSettings.IsMVC;
var aN=b.isSSLConnection?"wss://":"ws://";
if(aL){return aN+window.location.host+"/"+TXSettings.ApiRoute+"TXWebSocket";
}else{var aM=b.getHTTPHandlerRootDir();
return aN+aM+"TXWebSocketHandler.ashx";
}}b.init=function(){var aN=document.getElementById("mainCanvas");
var aL=document.getElementById("horRulerCanvas");
var aP=document.getElementById("vertRulerCanvas");
var aO=document.getElementById("statusBarCanvas");
aN.width=window.innerWidth;
aN.height=window.innerHeight;
aL.width=window.innerWidth;
aP.height=window.innerHeight;
aO.width=window.innerWidth;
b.textViewer=new b.ComponentViewer(aN,b.ComponentType.TextView);
b.horRulerViewer=new b.ComponentViewer(aL,b.ComponentType.HorRuler);
b.vertRulerViewer=new b.ComponentViewer(aP,b.ComponentType.VertRuler);
b.statusBarViewer=new b.ComponentViewer(aO,b.ComponentType.StatusBar);
b.webSocketHandlerURL=p();
console.info("Connecting to "+b.webSocketHandlerURL);
b.socket=new WebSocket(b.webSocketHandlerURL,"TXTextControl.Web");
b.socket.addEventListener("open",aK);
b.socket.addEventListener("message",aJ);
b.socket.addEventListener("error",aI);
b.socket.addEventListener("close",aG);
...
document.addEventListener("pongMessageReceived",x);
document.addEventListener("documentSaved",j);
document.addEventListener("textFieldDataReceived",b.textFieldDataReceivedHandler);
aN.addEventListener("drop",b.handleDrop);
aN.addEventListener("dragenter",function(aQ){aQ.preventDefault();
aQ.stopPropagation();
});
aN.addEventListener("dragover",function(aQ){aQ.preventDefault();
aQ.stopPropagation();
aQ.dataTransfer.dropEffect="copy";
});
document.getElementById("ribbonbar").addEventListener("contextmenu",y);
document.getElementById("vertical-scrollbar-wrapper").addEventListener("contextmenu",y);
document.getElementById("horizontal-scrollbar-wrapper").addEventListener("contextmenu",y);
document.getElementById("scroll-button-up").addEventListener("contextmenu",y);
document.getElementById("scroll-button-down").addEventListener("contextmenu",y);
document.getElementById("scroll-button-left").addEventListener("contextmenu",y);
document.getElementById("scroll-button-right").addEventListener("contextmenu",y);
document.addEventListener("mousedown",h);
document.addEventListener("mouseup",i);
document.addEventListener("click",g);
document.addEventListener("mouseout",function(aQ){if(!aQ.relatedTarget||(aQ.relatedTarget.nodeName=="HTML")){r.Left=false;
r.Middle=false;
r.Right=false;
if(b.mouseOwner){b.mouseOwner.canvas_MouseUp(aQ);
b.mouseOwner=null;
}}});
b.initContextMenus();
b.createScrollbars();
...
if(!b.isTouchDevice){b.textViewer.focus();
}var aM=document.getElementById("txLoaderGif");
aM.style.opacity=1;
ar();
};
a(document).ready(function(){b.init();
});
function c(){var aM=window.MutationObserver||window.WebKitMutationObserver||window.MozMutationObserver;
var aN=new aM(function(aO){aO.forEach(function(aP){switch(aP.type){case"attributes":switch(aP.attributeName){case"style":e();
break;
}break;
}});
});
var aL={attributes:true,childList:false,characterData:false};
aN.observe(document.getElementById("txTemplateDesignerContainer").parentElement,aL);
}function at(){if(typeof TXWebResources=="undefined"){return;
}if(!TXWebResources.hasOwnProperty("connectionID")){return;
}b.sendCommand(b.Command.SetGUID,0,0,0,TXWebResources.connectionID);
}function aa(){if((typeof TXSettings==="undefined")||!TXSettings.IsMVC){return;
...
}function n(aL){if(navigator.userLanguage){aL.culture=navigator.userLanguage;
aL.uiCulture=navigator.browserLanguage;
}else{var aM=navigator.languages?navigator.languages[0]:navigator.language;
if(aM){aL.culture=aL.uiCulture=aM;
}else{console.warn("Unable to detect browser language.");
}}}function o(){var aL={culture:"",uiCulture:""};
n(aL);
if(typeof TXWebResources!=="undefined"){if(TXWebResources.hasOwnProperty("culture")&&TXWebResources.hasOwnProperty("uiCulture")){if(TXWebResources.culture){aL.culture=TXWebResources.culture;
}if(TXWebResources.uiCulture){aL.uiCulture=TXWebResources.uiCulture;
}}}else{if(typeof TXSettings!=="undefined"){if(TXSettings.Culture){aL.culture=TXSettings.Culture;
}if(TXSettings.UICulture){aL.uiCulture=TXSettings.UICulture;
}}}return aL;
}function aK(){at();
var aL=o();
b.sendCommand(b.Command.CreateTextControl,0,0,0,aL);
b.sendCommand(b.Command.IsTouchDevice,b.isTouchDevice?1:0);
aa();
}function v(){e();
}function e(){var aL=document.getElementById("txTemplateDesignerContainer").parentElement;
var aO,aN;
if(aL.tagName=="BODY"){aO=window.innerWidth;
aN=window.innerHeight;
}else{aO=aL.offsetWidth;
aN=aL.offsetHeight;
}if((aO!=b.clientSize.Width)||(aN!=b.clientSize.Height)){var aM=new CustomEvent("txContainerResized");
document.dispatchEvent(aM);
}}b.blockUserInput=function(){var aL=document.getElementById("txTemplateDesignerContainer");
var aM=document.createElement("DIV");
aM.style.position="absolute";
aM.style.left=aM.style.top="0px";
aM.style.width=aM.style.height="100%";
aM.style.background="rgba(0, 0, 0, 0.1)";
aM.style.zIndex="9998";
aL.appendChild(aM);
};
function k(aM,aN,aL,aO){aM.style.transition="opacity "+(aO/1000)+"s";
setTimeout(function(){aM.style.opacity=0;
setTimeout(function(){aN.removeChild(aM);
},aO);
},aL);
}b.showMsgDiv=function(aO,aN){b.textViewer.hideCaret();
var aL=document.getElementById("txTemplateDesignerContainer");
var aM=document.createElement("DIV");
aM.id="txErrMsgDiv";
aM.classList.add("unselectable");
aM.style.zIndex="9999";
aO=aO.replace(/\r?\n/g,"<br />");
aM.innerHTML=aO;
aL.appendChild(aM);
aM.style.marginTop=-(aM.clientHeight/2)+"px";
aM.style.marginLeft=-(aM.clientWidth/2)+"px";
if(aN){k(aM,aL,1000,1000);
}};
b.showErrMsgDiv=function(){b.showMsgDiv(b.variousMessages.webSocketErrMsg.text);
};
function aD(){b.blockUserInput();
b.showErrMsgDiv();
}function aI(aL){b.socket=null;
setTimeout(aD,1000);
}function aG(aL){aI();
}function aJ(aL){var aN=""+aL.data;
var aM=JSON.parse(aN);
Y(aM);
}function Y(aN){if(!aN.hasOwnProperty("ID")){console.error("Message has no ID");
return;
}if(!aN.hasOwnProperty("Body")){console.error("Message has no message body");
return;
}switch(aN.ID){case d.FontList:L(aN.Body);
break;
case d.FileList:K(aN.Body);
break;
case d.StyleList:al(aN.Body);
break;
case d.Image:P(aN.Body);
break;
case d.PrintResult:ab(aN.Body);
break;
case d.FontSizes:M(aN.Body);
break;
case d.InputFormat:R(aN.Body);
break;
case d.DocumentSizeChanged:G(aN.Body);
break;
case d.Zoomed:aq(aN.Body);
break;
case d.InputPositionChanged:S(aN.Body);
break;
case d.TextViewLocation:am(aN.Body);
break;
case d.TextMessage:var aO=b.txDialog?b.txDialog.getActiveViewer():b.textViewer;
b.documentSavedEventHandlers=[];
b.jQueryAlert(aN.Body.caption,aN.Body.message,aO);
break;
case d.DBScheme:D(aN.Body);
break;
case d.SetCursor:ah(aN.Body);
break;
case d.DocumentTargets:H(aN.Body);
break;
case d.Item:U(aN.Body);
break;
case d.DialogResources:E(aN.Body);
break;
case d.SetAppMode:ag(aN.Body);
break;
case d.ShowDialog:aj(aN.Body);
break;
case d.FieldDisplayModeChanged:J(aN.Body);
break;
case d.SetRibbonButtonState:ai(aN.Body);
break;
case d.WaitDialog:ao(aN.Body);
break;
case d.SelectMasterTable:ae(aN.Body);
break;
case d.MergeFieldHighlightingChanged:X(aN.Body);
break;
case d.MergeBlockHighlightingChanged:W(aN.Body);
break;
case d.CaretStateChanged:z(aN.Body);
break;
case d.CloseDialog:A(aN.Body);
break;
case d.ImageRegions:Q(aN.Body);
break;
case d.MoveAndUpdate:Z(aN.Body);
break;
case d.ContextMenuData:C(aN.Body);
break;
case d.ConsoleLog:B(aN.Body);
break;
case d.DrawShape:I(aN.Body);
break;
case d.FieldPropertiesDialog:var aL=new CustomEvent("showFieldPropsDialog");
document.dispatchEvent(aL);
break;
case d.HyperlinkClicked:O(aN.Body);
break;
case d.Pong:aL=new CustomEvent("pongMessageReceived");
document.dispatchEvent(aL);
break;
case d.UnknownProtocol:an();
break;
case d.StringResources:ak(aN.Body);
break;
case d.ProofingSettings:ac(aN.Body);
break;
case d.ItemActivationStateChangedEvent:T(aN.Body);
break;
case d.HTMLDialogState:N(aN.Body);
break;
case d.DocumentDirtyChanged:b.documentDirty=aN.Body.documentDirty;
break;
case d.DocumentSaved:var aM=aN.Body.message;
if(aM&&aM.length){b.showMsgDiv(aM,true);
}b.documentDirty=false;
aL=new CustomEvent("documentSaved");
document.dispatchEvent(aL);
break;
case d.EnableSaveDocumentQuestion:b.saveDocumentQuestionEnabled=aN.Body.enabled;
break;
case d.SetTextViewActivationState:au(aN.Body.activationState);
break;
case d.EditMode:b.setEditMode(aN.Body.editMode);
break;
case d.ImageResources:b.imageResources=aN.Body;
break;
case d.MenuContent:V(aN.Body);
break;
case d.WSDisconnectReason:ap(aN.Body);
break;
case d.ServerEvent:af(aN.Body);
break;
case d.DocumentData:F(aN.Body);
break;
case d.SelectionData:ad(aN.Body);
break;
}}function au(aL){switch(aL){case b.ActivationState.Activated:b.textViewer.focus();
break;
case b.ActivationState.Deactivated:b.textViewer.blur();
break;
}}function P(aM){var aL=new CustomEvent("imageReceived",{detail:aM});
document.dispatchEvent(aL);
}function L(aM){var aL=new CustomEvent("fontListReceived",{detail:{fontNames:aM.fontNames}});
document.dispatchEvent(aL);
}function f(aM){var aL=new CustomEvent("uiFlagsUpdated",{detail:aM});
document.dispatchEvent(aL);
}function K(aM){var aL=new CustomEvent("fileListReceived",{detail:aM});
document.dispatchEvent(aL);
}function al(aM){var aL=new CustomEvent("styleListReceived",{detail:{styleNames:aM.styleNames}});
document.dispatchEvent(aL);
}function ab(aM){var aL=new CustomEvent("printResultReceived",{detail:aM});
document.dispatchEvent(aL);
}function M(aM){var aL=new CustomEvent("fontSizesReceived",{detail:{fontSizes:aM.fontSizes}});
document.dispatchEvent(aL);
}function R(aM){var aL=new CustomEvent("inputFormatReceived",{detail:aM});
document.dispatchEvent(aL);
}function G(aM){var aL=new CustomEvent("documentSizeChanged",{detail:aM});
document.dispatchEvent(aL);
}function aq(aM){var aL=new CustomEvent("txZoomed",{detail:aM});
document.dispatchEvent(aL);
}function S(aM){var aL=new CustomEvent("inputPositionChanged",{detail:aM});
document.dispatchEvent(aL);
f(aM.uiFlags);
}function am(aM){var aL=new CustomEvent("textViewLocationChanged",{detail:aM});
document.dispatchEvent(aL);
f(aM.uiFlags);
}function D(aM){var aL=new CustomEvent("dbSchemeReceived",{detail:aM});
document.dispatchEvent(aL);
}function ah(aM){var aL=new CustomEvent("componentCursorChanged",{detail:aM});
document.dispatchEvent(aL);
}function H(aM){var aL=new CustomEvent("documentTargetsReceived",{detail:{targets:aM.targets}});
document.dispatchEvent(aL);
}function U(aM){var aL=new CustomEvent("txItemReceived",{detail:aM});
document.dispatchEvent(aL);
switch(aM.itemType){case b.ItemType.FieldAdapter:aL=new CustomEvent("fieldDataReceived",{detail:aM.item});
document.dispatchEvent(aL);
break;
case b.ItemType.TextField:aL=new CustomEvent("textFieldDataReceived",{detail:aM});
document.dispatchEvent(aL);
break;
}}function E(aM){var aL=new CustomEvent("dialogResourcesReceived",{detail:aM});
document.dispatchEvent(aL);
}function ag(aM){var aL=new CustomEvent("appModeChanged",{detail:aM});
document.dispatchEvent(aL);
}function aj(aL){switch(aL.dialogCategory){case b.DialogCategory.HTMLDialog:ay(aL);
break;
case b.DialogCategory.TXDialog:if(!b.txDialog){b.txDialog=new b.TxDialog(aL.initialImage,aL.caption,aL.dialogType);
b.txDialog.showModal();
}else{b.txDialog.showChildDialog(aL.initialImage,aL.caption);
}break;
}}function ay(aM){var aL;
switch(aM.dialogType){case b.HTMLDialogType.InputBoxDialog:az(aM);
break;
case b.HTMLDialogType.MaxPreviewRowsDialog:aA(aM);
break;
case b.HTMLDialogType.ChildDialog:if(b.txDialog){b.txDialog.showChildDialog(aM.initialImage,aM.caption);
}else{b.jQueryAlert("Error Message","Can not open child dialog without having any TXTextControl dialog open.");
}break;
case b.HTMLDialogType.EditDataRelationsDialog:aw(aM.relations);
break;
case b.HTMLDialogType.EditMergeBlocksDialog:ax(aM.blocks);
break;
case b.HTMLDialogType.OptionsDialog:aB(aM.options);
break;
case b.HTMLDialogType.ChartSettingsDialog:av(aM.properties);
break;
case b.HTMLDialogType.InsertMergeBlockDialog:aL=new b.InsertMergeBlockDialog(aM.name,b.dbScheme);
aL.showModal();
break;
case b.HTMLDialogType.SaveDocumentAsDialog:aC(aM.fileName,aM.streamType);
break;
case b.HTMLDialogType.SpellCheckDialog:aL=new b.SpellCheckDialog(aM);
aL.showModal();
break;
case b.HTMLDialogType.LoadDictionaryDialog:aL=new b.LoadDictionaryDialog(aM);
aL.showModal();
break;
}}function J(aM){var aL=new CustomEvent("fieldDisplayModeChanged",{detail:aM});
document.dispatchEvent(aL);
}function ai(aM){var aL=new CustomEvent("ribbonButtonStateMessageReceived",{detail:aM});
document.dispatchEvent(aL);
}function ao(aM){var aL=new CustomEvent("waitDialogMessageReceived",{detail:aM});
document.dispatchEvent(aL);
}function ae(aM){var aL=new CustomEvent("masterTableSelected",{detail:aM});
document.dispatchEvent(aL);
}function X(aM){var aL=new CustomEvent("mergeFieldHighlightingChanged",{detail:aM});
document.dispatchEvent(aL);
}function W(aM){var aL=new CustomEvent("mergeBlockHighlightingChanged",{detail:aM});
document.dispatchEvent(aL);
}function z(aM){var aN=aM.target;
switch(aN){case b.ComponentType.TxDialog:var aL=new CustomEvent("dialogCaretStateChanged",{detail:aM});
document.dispatchEvent(aL);
break;
case b.ComponentType.TextView:var aL=new CustomEvent("textViewCaretStateChanged",{detail:aM});
document.dispatchEvent(aL);
break;
}}function A(aM){var aL=new CustomEvent("txDialogClosed",{detail:aM});
document.dispatchEvent(aL);
}function Q(aM){var aL=new CustomEvent("imageRegionsUpdated",{detail:aM});
document.dispatchEvent(aL);
}function Z(aM){var aL=new CustomEvent("viewContentMovedAndUpdated",{detail:aM});
document.dispatchEvent(aL);
}function C(aM){var aL=new CustomEvent("contextMenuDataReceived",{detail:aM});
document.dispatchEvent(aL);
}function B(aL){var aM=aL.text;
switch(aL.type){case b.ConsoleLogType.Log:console.log(aM);
break;
case b.ConsoleLogType.Debug:console.debug(aM);
break;
case b.ConsoleLogType.Info:console.info(aM);
break;
case b.ConsoleLogType.Warn:console.warn(aM);
break;
case b.ConsoleLogType.Error:console.error(aM);
break;
}}function I(aM){var aL=new CustomEvent("drawShapeMessageReceived",{detail:aM});
document.dispatchEvent(aL);
}function O(aM){var aL=new CustomEvent("hyperlinkClicked",{detail:aM.link});
document.dispatchEvent(aL);
}function an(){}function ak(aL){var aM=aL.resources;
switch(aL.type){case aE.VariousMessages:b.variousMessages=aM;
break;
case aE.RibbonStrings:b.ribbonStringResources=aM;
b.initRibbon();
break;
}}function ac(aL){if(aL.isSpellCheckingEnabled){a("#btnSpelling").removeClass("ui-state-disabled");
...
}if(aL.isLangDetectEnabled){a("#tglBtnEnableLangDetect").addClass("ribbon-button-selected");
}else{a("#tglBtnEnableLangDetect").removeClass("ribbon-button-selected");
}if(aL.isHyphenationEnabled){a("#tglBtnEnableHyphenation").addClass("ribbon-button-selected");
}else{a("#tglBtnEnableHyphenation").removeClass("ribbon-button-selected");
}}function T(aM){var aL=null;
switch(aM.itemType){case b.ItemType.HeaderFooter:switch(aM.activationState){case b.ItemActivationState.Activated:aL=new CustomEvent("headerFooterActivated");
break;
case b.ItemActivationState.Deactivated:aL=new CustomEvent("headerFooterDeactivated");
break;
}break;
}if(aL){document.dispatchEvent(aL);
}}function N(aM){switch(aM.dialogType){case b.HTMLDialogType.SpellCheckDialog:var aL=new CustomEvent("spellCheckDialogStateUpdated",{detail:aM.state});
document.dispatchEvent(aL);
break;
}}function V(aM){var aL;
switch(aM.menuType){case b.MenuType.InsertShapes:b.insertShapesMenuContent=aM.menuItems;
break;
}}function ap(aL){var aM="";
switch(aL.reason){case aH.CouldNotConnectToService:aM="The web server could not connect to the\r\nTX Text Control service for unknown reasons.";
break;
case aH.ServiceNotInstalled:aM="The web server could not connect to the TX Text Control service\r\nbecause the service is not installed.";
break;
case aH.ServiceNotRunning:aM="The web server could not connect to the TX Text Control service\r\nbecause the service is not running.";
break;
}if(aM){b.variousMessages.webSocketErrMsg.text=aM;
}console.error(aL.excType+": "+aL.excMsg);
}function af(aL){switch(aL.eventType){case b.ServerEventType.DocumentLoaded:l(aL.eventData);
break;
}}function l(aM){var aL={fileName:aM.FileName};
b.fireEvent(b.EventName.documentLoaded,aL);
}function F(aP){var aM=b.saveDocumentCallbacks;
b.saveDocumentCallbacks=[];
if(aP.excMsg){throw Error(aP.excMsg);
}for(var aO=0;
aO<aM.length;
++aO){var aL=aM[aO];
try{aL({data:aP.data,streamType:aP.streamType});
}catch(aN){}}}function ad(aP){var aM=b.saveSelectionCallbacks;
...
++aO){var aL=aM[aO];
try{aL({data:aP.data,streamType:aP.streamType});
}catch(aN){}}}b.fileSaveQuestion=function(aN){aN=aN||function(){};
if(!b.saveDocumentQuestionEnabled){aN();
return;
}var aR=b.txDialog?b.txDialog.getActiveViewer():b.textViewer;
var aM=b.variousMessages.fileSaveQuestionDlg.caption;
var aQ=b.variousMessages.fileSaveQuestionDlg.text;
var aS=b.variousMessages.fileSaveQuestionDlg.yesBtnTxt;
var aO=b.variousMessages.fileSaveQuestionDlg.noBtnTxt;
var aL=b.variousMessages.fileSaveQuestionDlg.cancelBtnTxt;
b.documentSavedEventHandlers=[];
var aP=function(){b.sendCommand(b.Command.Save);
b.documentSavedEventHandlers.push(aN);
};
a('<div style="font-size: 10pt; min-width: 250px">'+aQ+"</div>").dialog({modal:true,resizable:false,title:aM,width:"auto",height:130,buttons:{Yes:{text:aS,click:function(){a(this).dialog("destroy").remove();
aP();
if(aR&&!b.isTouchDevice){aR.focus();
}}},No:{text:aO,click:function(){a(this).dialog("destroy").remove();
aN();
if(aR&&!b.isTouchDevice){aR.focus();
}}},Cancel:{text:aL,click:function(){a(this).dialog("destroy").remove();
if(aR&&!b.isTouchDevice){aR.focus();
}}}},close:function(){a(this).dialog("destroy").remove();
if(aR&&!b.isTouchDevice){aR.focus();
}}});
};
function az(aL){var aM=new b.InputBoxDialog(aL.title,aL.text);
aM.showModal();
}function aA(aL){var aM=new b.MaxPreviewRowsDialog(aL.value);
aM.showModal();
}function aw(aM){var aL=new b.EditDataRelationsDialog(aM);
aL.showModal();
}function ax(aL){var aM=new b.EditMergeBlocksDialog(aL);
aM.showModal();
}function aB(aM){var aL=new b.OptionsDialog(aM);
aL.showModal();
}function av(aM){var aL=new b.ChartSettingsDialog(aM);
aL.showModal();
}function aC(aM,aN){var aL=new b.SaveDocumentAsDialog(aM,aN);
aL.showModal();
}function m(aL){if(b.isTouchDevice){return;
}var aM=document.getElementById("txTemplateDesignerContainer");
if((aL==aM)||(aM.contains(aL))){if((aL.tagName!=="SELECT")&&(aL.tagName!=="INPUT")){b.textViewer.focus();
}}}function s(aL){var aM=document.getElementsByClassName("scrollbar-wrapper");
for(var aN=0;
aN<aM.length;
++aN){if(aM[aN].contains(aL)){m(aL);
break;
}}}function h(aL){s(aL.target);
r.Left=(aL.which==1);
r.Middle=(aL.which==2);
r.Right=(aL.which==3);
}function i(aL){if(aL.which==1){r.Left=false;
}if(aL.which==2){r.Middle=false;
}if(aL.which==3){r.Right=false;
}if(!r.Left&&!r.Middle&&!r.Right){b.mouseOwner=null;
}}function g(aL){m(aL.target);
}function x(){}function j(){for(var aM=0;
aM<b.documentSavedEventHandlers.length;
++aM){var aL=b.documentSavedEventHandlers[aM];
aL();
}b.documentSavedEventHandlers=[];
}function y(aL){aL.preventDefault();
}b.webSocketDisconnect=function(){if(!b.socket){return;
}if(b.isSocketConnected()){b.socket.removeEventListener("error",aI);
b.socket.removeEventListener("close",aG);
b.socket.close();
}b.socket=null;
};
function t(aL){if(b.isTouchDevice&&b.variousMessages.onBeforeUnloadMsg){return b.variousMessages.onBeforeUnloadMsg;
}}function u(aL){b.sendCommand(b.Command.Close);
b.webSocketDisconnect();
}b.jQueryAlert=function(aL,aM,aN){aL=aL||"";
aM=aM||"";
aM=aM.replace(/\r?\n/g,"<br />");
aN=typeof aN!=="undefined"?aN:null;
a('<div style="font-size: 10pt; min-width: 250px">'+aM+"</div>").dialog({modal:true,resizable:false,title:aL,width:"auto",buttons:{OK:function(){a(this).dialog("destroy").remove();
if(aN&&!b.isTouchDevice){aN.focus();
}},},close:function(){a(this).dialog("destroy").remove();
if(aN&&!b.isTouchDevice){aN.focus();
}}});
};
...
}function k(){a("#ribbonTabHome").load("/TXTextControl.Web/Ribbon/RibbonTabHome.html",function(){b.attachHomeTabEventHandlers();
a("#ribbonTabInsert").load("/TXTextControl.Web/Ribbon/RibbonTabInsert.html",function(){b.attachInsertTabEventHandlers();
a("#ribbonTabPageLayout").load("/TXTextControl.Web/Ribbon/RibbonTabPageLayout.html",function(){b.attachPageLayoutTabEventHandlers();
a("#ribbonTabMailings").load("/TXTextControl.Web/Ribbon/RibbonTabMailings.html",function(){b.attachMailingsTabEventHandlers();
a("#ribbonTabView").load("/TXTextControl.Web/Ribbon/RibbonTabView.html",function(){b.attachViewTabEventHandlers();
a("#ribbonTabTableLayout").load("/TXTextControl.Web/Ribbon/RibbonTabTableLayout.html",function(){b.attachTableLayoutTabEventHandlers();
a("#ribbonTabObjectFormat").load("/TXTextControl.Web/Ribbon/RibbonTabObjectFormat.html",function(){b.attachObjectFormatTabEventHandlers();
a("#ribbonTabShapeFormat").load("/TXTextControl.Web/Ribbon/RibbonTabShapeFormat.html",function(){b.attachShapeFormatTabEventHandlers();
i();
e();
b.setRibbonStrings();
});
